...
int main()
{
    freopen("data.txt", "r", stdin);    // input from the file "data.txt",which is more convenient for testing.
    freopen("output.txt", "w", stdout);
    long long sum;                      // avoid the overflow of integer.
    int n1, n2; // 
    while (scanf("%d", &n1) != EOF)
    {
        int root1 = -1, root2 = -1;//root is initialized as -1,which means we haven't found the root yet.
